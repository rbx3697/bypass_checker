...
def check():
    ...
    try:
        ...
        flash("Ошибка выполнения проверки.", "danger")
        return redirect(url_for("index"))

    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)

    # распакуем для красивого UI
...
def _build_summary_line(payload: dict, secret_cookie: str | None) -> str:
    ...
    billing = ",".join(c.get("billing_sources", [])) if c.get("billing_sources") else "Нет"
    card = "Нет"  # если нужно, распарсим из billing_sources тип "card"
...
def export_txt(task_id):
    # include_secret=1 добавит в конец куки (ОПАСНО)
